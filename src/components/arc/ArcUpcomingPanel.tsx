  ...
  return (
    <div className="bg-amber-500/10 border border-amber-500/20 rounded px-3 py-2">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-amber-400 font-mono font-bold tabular-nums">
          {formatCountdownShort(arc.timeToStart)}
        </span>
      </div>
      <div className="flex items-center justify-between mt-1">
        <span className="text-[10px] text-amber-400">{cityName}</span>
...
export const ArcUpcomingPanel: React.FC<ArcUpcomingPanelProps> = ({
  ...
  return (
    <div className="fixed top-20 left-0 z-20 w-80 bg-[#020617]/60 backdrop-blur border border-amber-500/30 rounded-t-lg rounded-b-none shadow-lg">
      {/* 面板标题 */}
